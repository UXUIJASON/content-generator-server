...
class ContentRewriter {
  ...
  async rewriteContent(originalContent, data) {
    try {
      const versions = await Promise.all([
        ...
        this.generateVersion(originalContent, 3, data),
        this.generateVersion(originalContent, 4, data),
        this.generateVersion(originalContent, 5, data)
      ]);
  ...
  async generateVersion(originalContent, versionNumber, data) {
    const prompt = this.buildRewritePrompt(originalContent, versionNumber, data);
